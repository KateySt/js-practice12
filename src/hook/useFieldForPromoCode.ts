...
function useFieldForPromoCode() {
    const [{promoCode}, setText] = useState<ISubmitResult>({promoCode: '48183276'});

    const onChangeSetText = (event: ChangeEvent<{ value: string }>) => {
        setText({promoCode: event.target.value});
    }
...
export default useFieldForPromoCode;